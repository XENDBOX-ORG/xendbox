...
export async function receiveParcel(stationId: string, orderId: string) {
  const station = await prisma.pickupStation.findUnique({ where: { id: stationId } })
  if (!station) throw new AppError("Pickup station not found", 404)

  const order = await prisma.order.findUnique({ where: { id: orderId } })
  if (!order) throw new AppError("Order not found", 404)

  const existing = await prisma.stationParcel.findUnique({ where: { order_id: orderId } })
  if (existing) throw new AppError("Parcel already received at a station", 409)

  const parcel = await prisma.stationParcel.create({
...
export async function markReadyForCollection(parcelId: string, stationId: string) {
  const parcel = await prisma.stationParcel.findFirst({
    where: { id: parcelId, station_id: stationId },
  })
  if (!parcel) throw new AppError("Parcel not found at this station", 404)

  const updated = await prisma.stationParcel.update({
    where: { id: parcelId },
...
export async function collectParcel(
  ...
  await prisma.pickupVerificationLog.create({
    data: {
  ...
  await prisma.stationParcel.update({
    where: { id: parcelId },
...
export async function generatePickupCode(orderId: string, consumerId: string) {
  const order = await prisma.order.findFirst({
    ...
    include: { pickup_code: true },
  })
  if (!order) throw new AppError("Order not found", 404)
  if (order.pickup_code) throw new AppError("Pickup code already generated", 409)
